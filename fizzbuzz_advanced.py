...
def num_div_1_to_9(n):
    return [int((n % x) == 0) for x in range(1,10)]


def make_training_set(size):
    data = []
    targets = []
...
def fizzbuzz_ml(clf, n):
    prediction, = clf.predict([num_div_1_to_9(n)])
...
if __name__ == '__main__':
    ...
    X, Y = make_training_set(training_size)
    clf = RandomForestClassifier(n_estimators=10)
    clf = clf.fit(X, Y)

    print("Trained on integers [1,{}]".format(training_size))

    success = 0
    failures = 0
    for n in range(1, N):
        #print("{}: {}/{}".format(n, fizzbuzz_ml(clf, n), fizzbuzz(n)))
        ml_result = fizzbuzz_ml(clf, n)
